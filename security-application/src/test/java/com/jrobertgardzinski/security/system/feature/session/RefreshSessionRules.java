...
public class RefreshSessionRules {
    ...
    private static final int REFRESH_TOKEN_VALIDITY_HOURS = 1;
    ...
    public RefreshSessionRules(StubAuthorizationDataRepository authorizationDataRepository) {
        ...
        this.refreshSession = new RefreshSession(authorizationDataRepository, clock,
                REFRESH_TOKEN_VALIDITY_HOURS, anyInt());
    }
    ...
    public void givenActiveSession() {
        SessionTokens sessionTokens = SessionTokens.createFor(email,
                REFRESH_TOKEN_VALIDITY_HOURS, anyInt(), clock);
    ...
    public void givenExpiredSession() {
        Clock pastClock = Clock.fixed(
                LocalDateTime.now().minusHours(REFRESH_TOKEN_VALIDITY_HOURS + 1).atZone(ZoneId.systemDefault()).toInstant(),
                ZoneId.systemDefault());
        SessionTokens session = SessionTokens.createFor(email,
                REFRESH_TOKEN_VALIDITY_HOURS, anyInt(), pastClock);
