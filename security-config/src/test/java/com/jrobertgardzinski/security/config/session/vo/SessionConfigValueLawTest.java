...
/**
 * A law over this package, the twin of the one over the brute-force limits: every session ceiling
 * honours
 ...
 * new ceiling cannot forget one in silence.
 */
@Epic("Security")
@Feature("Configuration")
@Story("Every session ceiling knows its key, its value and the default the code ships")
class SessionConfigValueLawTest {

    private static final Path SOURCES = Path.of("src/main/java/com/jrobertgardzinski/security/config/session/vo");

    @Test
    void every_ceiling_is_a_ConfigValue_with_a_static_DEFAULT_and_KEY_that_agree_with_the_instance() throws Exception {
    ...
    private static List<Class<?>> typesInThePackage() throws IOException {
        try (Stream<Path> files = Files.list(SOURCES)) {
            return files.filter(file -> file.toString().endsWith(".java"))
                    ...
                    .<Class<?>>map(name -> {
                        try {
                            return Class.forName("com.jrobertgardzinski.security.config.session.vo." + name);
