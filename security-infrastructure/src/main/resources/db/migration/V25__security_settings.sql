-- The RUNTIME level of the layered configuration ladder (shared/config): one row per key, the key
-- string identical to the property name, so the very same name is visible on every level of the
-- ladder — application.yml, this table. No row means the level is VACANT and resolution falls
...
-- level (the ladder falls through) — a hand-edited row must never take password validation down.
--
