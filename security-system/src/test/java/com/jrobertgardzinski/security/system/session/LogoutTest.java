...
class LogoutTest {
    ...
    void revokes_the_family() {
        Mockito.when(sessionRepository.findByRefreshToken(TOKEN)).thenReturn(Optional.of(
    ...
    void unknown_session_is_a_no_op() {
        Mockito.when(sessionRepository.findByRefreshToken(TOKEN)).thenReturn(Optional.empty());
