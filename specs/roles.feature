...
@http-only
Feature: The cast of the estate — GUEST, USER, MODERATOR, ADMIN

  Identity's ladder, spoken once for every product. A GUEST is whoever carries no
  identity: security never meets them, and what a GUEST may see is each product's
  own rule. Signing in makes a USER, and every USER holds the USER role. MODERATOR
  and ADMIN are granted on top — only by an ADMIN's hand — and the who-am-I
  resource reports the caller's ROLES, so every other service can gate on them
  without guessing.
